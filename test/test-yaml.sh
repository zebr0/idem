...
../zebr0-script -c tmp -d tmp/history run test-ok --dry > tmp/dry_before
diff tmp/dry_before results/dry_before

# test first run
../zebr0-script -c tmp -d tmp/history run test-ok > tmp/first_run
...
../zebr0-script -c tmp -d tmp/history run test-ok --dry > tmp/dry_after
diff tmp/dry_after results/dry_after

# test second run
../zebr0-script -c tmp -d tmp/history run test-ok > tmp/second_run
diff tmp/second_run results/second_run

# test script ko
../zebr0-script -c tmp -d tmp/history run test-ko-script > tmp/ko-script || true
diff tmp/ko-script results/ko-script

# test lookup ko
../zebr0-script -c tmp -d tmp/history run test-ko-lookup > tmp/ko-lookup || true
...
# test connection ko
../zebr0-script -c tmp -d tmp/history run test-ko-connection > tmp/ko-connection-dirty || true
