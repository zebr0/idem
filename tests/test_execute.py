...
def test_ko(monkeypatch, capsys):
    ...
    assert not zebr0_script.execute("./global-thermonuclear-war", attempts=3, pause=0.1)
...
def test_ko_then_ok(monkeypatch, capsys):
    ...
    assert zebr0_script.execute("./global-thermonuclear-war", pause=0.1) == "./global-thermonuclear-war"
