...
def test_ok_status_failure(server, tmp_path, capsys):
    report_path = tmp_path.joinpath("a71c44f4d35f54f68fe8930a1c29148f")
    ...
    result = zebr0_script.recursive_fetch_script(client, "script", tmp_path)
    assert list(result) == [("install package yyy", zebr0_script.Status.FAILURE, report_path)]
    assert capsys.readouterr().out == ""


def test_ok_status_unknown(server, tmp_path, capsys):
    report_path = tmp_path.joinpath("a71c44f4d35f54f68fe8930a1c29148f")
    ...
    result = zebr0_script.recursive_fetch_script(client, "script", tmp_path)
    assert list(result) == [("install package yyy", "unknown", report_path)]
    assert capsys.readouterr().out == ""
